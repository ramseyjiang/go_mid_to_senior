...
func EntryInstance3() {
	...
	// this can be done by calling `getSquareChan` function multiple times where individual function call returns a channel
	// which sends square of numbers provided by `chanInputNums` channel
	// `getSquareChan` function runs goroutines internally where squaring operation is running concurrently
	...
	// that happen in `merge` function when all goroutines pushing to merged channel finishes
...
func merge(outputsChan ...<-chan int) <-chan int {
	...
	// when n is equal to number of channels received as argument the function
