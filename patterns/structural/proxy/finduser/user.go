...
// The UserFinder is the interface that the database and the Proxy implement.
...
// FindUser will iterate over the list to try to find a user with the same ID that the param or return an error if it can't find it
...
// UserListProxy is the proxy type, will be composed of a UserList slice, which will be our database representation.
